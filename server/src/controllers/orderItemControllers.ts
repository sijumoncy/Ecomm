...
const addOrderItemController = async (req: Request, res: Response) => {
...
const getOrderItemsController = async (req: Request, res: Response) => {
  const filter = pickKeyValues(req.query, ['productId']);
  const options = pickKeyValues(req.query, ['limit', 'page']);
  const result = await getOrderItemsService(filter, options);
  res.status(httpStatus.OK).send(result);
};

const getOrderItemController = async (req: Request, res: Response) => {
  const orderItem = await getOrderItemByIdService(req.params.orderItemId);
  if (!orderItem) {
...
const updateOrderItemController = async (req: Request, res: Response) => {
  const orderItem = await updateOrderItemByIdService(req.params.orderItemId, req.body);
...
const deleteOrderItemController = async (req: Request, res: Response) => {
  const deletedOrderItem = await deleteOrderItemByIdService(req.params.orderItemId);
